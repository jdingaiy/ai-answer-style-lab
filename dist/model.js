...
export function preset(scene='web') {
  ...
  const config={width:card||mobile?390:840,padding:card||mobile?20:30,tightTitles:false,tight:card?4:8,itemGap:4,indent:25,markerGap:4,markerSize:6,markerColor:'#242a33',cellPadding:8,columnMin:80,columnMax:card?200:mobile?240:280,ruleColor:'#e2e5eb',ruleWidth:1};
...
export function buildCSS(c) {
  ...
  css+=`.markdown-body ul li{position:relative;list-style:none;}.markdown-body ul li::before{content:'•';position:absolute;right:calc(100% + ${c.markerGap}px);top:.85em;top:.5lh;transform:translateY(-50%);font-size:${c.markerSize}px;line-height:1;color:${c.markerColor};}\n`;
...
const parser=new Marked({gfm:true,breaks:false,renderer:{
  ...
  paragraph({tokens}){const text=this.parser.parseInline(tokens);const value=String(text||'').trim();if(/^<img\b[^>]*>$/i.test(value))return `<figure class="md-image">${value}</figure>`;return `<p>${text}</p>`;},
