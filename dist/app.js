...
for(const s of Object.keys(scenes))applyHeadingRhythm(configs[s]);
const shared=decodeShared(new URLSearchParams(location.search).get('share')||'');if(shared?.configs){for(const s of Object.keys(scenes))configs[s]=applyHeadingRhythm(normalize(shared.configs[s],s));if(typeof shared.source==='string')source=shared.source;activeVersion='';}
