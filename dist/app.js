...
function updateCompare(){const divider=$('compare-divider'),old=$('answer-legacy'),fresh=$('answer');if(!compare){$('frame').classList.remove('is-comparing');divider.style.left='50%';fresh.style.clipPath='';old.style.clipPath='';return;}$('frame').classList.add('is-comparing');divider.style.left=(divider.dataset.position||'50')+'%';fresh.style.clipPath=`inset(0 0 0 ${divider.dataset.position||50}%)`;old.style.clipPath=`inset(0 ${100-(Number(divider.dataset.position||50))}% 0 0)`;}
