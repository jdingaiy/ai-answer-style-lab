...
const STORAGE='ai-answer-style-lab-v1',VERSION_LIMIT=30,DEFAULT_CONFIG_VERSION='20260910-quote-module-spacing-v16';
...
function applyHeadingRhythm(c){const bodySize=c.body.size,bodyAfter=c.body.after,divider=c.hr.before,lineMultiplier=c.width<=420&&c.width>=380?1.6:1.7;c.body.line=lineMultiplier;c.body.unit='multiplier';const sizes=[bodySize+5,bodySize+3,bodySize+1,bodySize,bodySize,bodySize];const before=[divider,divider-4,divider-6,divider-8,divider-8,divider-8];const after=[bodyAfter+2,bodyAfter,bodyAfter-2,bodyAfter-4,bodyAfter-4,bodyAfter-4];for(let i=1;i<=6;i++){const h=c['h'+i];h.size=sizes[i-1];h.before=Math.max(0,before[i-1]);h.after=Math.max(0,after[i-1]);h.line=lineMultiplier;h.unit='multiplier';}for(const key of ['ol','ul','table','thead']){c[key].line=lineMultiplier;c[key].unit='multiplier';}c.ol.after=bodyAfter;c.ul.after=bodyAfter;for(const key of ['code','table','image']){c[key].before=bodyAfter+4;c[key].after=bodyAfter+4;}return c;}
