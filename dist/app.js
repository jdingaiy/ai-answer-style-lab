...
const STORAGE='ai-answer-style-lab-v1',VERSION_LIMIT=30,DEFAULT_CONFIG_VERSION='20260910-fixed-quote-spacing-v17';
...
function applyHeadingRhythm(c){const bodySize=c.body.size,bodyAfter=c.body.after,divider=c.hr.before,lineMultiplier=c.width<=420&&c.width>=380?1.6:1.7;c.body.line=lineMultiplier;c.body.unit='multiplier';const sizes=[bodySize+5,bodySize+3,bodySize+1,bodySize,bodySize,bodySize];const before=[divider,divider-4,divider-6,divider-8,divider-8,divider-8];const after=[bodyAfter+2,bodyAfter,bodyAfter-2,bodyAfter-4,bodyAfter-4,bodyAfter-4];for(let i=1;i<=6;i++){const h=c['h'+i];h.size=sizes[i-1];h.before=Math.max(0,before[i-1]);h.after=Math.max(0,after[i-1]);h.line=lineMultiplier;h.unit='multiplier';}for(const key of ['ol','ul','table','thead']){c[key].line=lineMultiplier;c[key].unit='multiplier';}c.ol.after=bodyAfter;c.ul.after=bodyAfter;c.quote.before=10;c.quote.after=10;for(const key of ['code','table','image']){c[key].before=bodyAfter+4;c[key].after=bodyAfter+4;}return c;}
...
window.addEventListener('unhandledrejection',event=>console.error('Unhandled export error',event.reason));
async function prepareExportImages(root){let failed=0;for(const image of root.querySelectorAll('img')){const src=image.currentSrc||image.src;if(!src||/^(data:|blob:)/.test(src))continue;try{const response=await fetch(src,{mode:'cors'});if(!response.ok)throw new Error('image');image.src=await blobToDataURL(await response.blob());}catch{failed++;const placeholder=document.createElement('span');placeholder.className='export-image-placeholder';placeholder.textContent=image.alt||'图片';image.replaceWith(placeholder);}}return failed;}
function exportStyles(){let css='';for(const sheet of document.styleSheets){try{css+=[...sheet.cssRules].map(rule=>rule.cssText).join('\n')+'\n';}catch{/* Ignore inaccessible third-party style sheets. */}}return css;}
async function exportPreviewJPG(){const button=$('export-jpg'),frame=$('frame');button.disabled=true;button.textContent='正在导出…';try{await document.fonts?.ready;const clone=frame.cloneNode(true);clone.querySelector('#answer-legacy')?.remove();clone.querySelector('#compare-divider')?.remove();clone.querySelector('.gap-overlay')?.remove();clone.classList.remove('is-comparing');const answer=clone.querySelector('#answer');if(answer){answer.style.clipPath='none';answer.classList.remove('measuring');}clone.removeAttribute('id');clone.style.maxWidth='none';clone.style.width=frame.getBoundingClientRect().width+'px';clone.style.minHeight='0';clone.style.height='auto';clone.style.margin='0';const failed=await prepareExportImages(clone);const width=Math.ceil(frame.getBoundingClientRect().width),height=Math.ceil(Math.max(frame.scrollHeight,clone.scrollHeight));clone.style.height=height+'px';const wrapper=document.createElementNS('http://www.w3.org/1999/xhtml','div'),style=document.createElementNS('http://www.w3.org/1999/xhtml','style');style.textContent=exportStyles();wrapper.append(style,clone);const xhtml=new XMLSerializer().serializeToString(wrapper);const markup=`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><foreignObject width="100%" height="100%">${xhtml}</foreignObject></svg>`;const svgURL='data:image/svg+xml;charset=utf-8,'+encodeURIComponent(markup);const raster=new Image();raster.src=svgURL;await raster.decode();const maxSide=16384,scale=Math.min(2,maxSide/Math.max(width,height));const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(width*scale));canvas.height=Math.max(1,Math.round(height*scale));const context=canvas.getContext('2d');context.scale(scale,scale);context.fillStyle='#ffffff';context.fillRect(0,0,width,height);context.drawImage(raster,0,0,width,height);const jpg=await new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('JPG')),'image/jpeg',.94));const url=URL.createObjectURL(jpg),link=document.createElement('a');link.href=url;link.download=`ai-answer-${scene}-${new Date().toISOString().slice(0,10)}.jpg`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notify(failed?`JPG 已导出，${failed} 张跨域图片使用了占位。`:'当前排版已导出为 JPG。');}catch(error){console.error('JPG export failed',error);notify(`JPG 导出失败：${error?.message||'未知错误'}`);}finally{button.disabled=false;button.textContent='导出 JPG';}}
function closeExportMenu(){const menu=$('export-menu-list'),button=$('export-menu-button');menu.hidden=true;button.setAttribute('aria-expanded','false');}
async function exportToFigma(){closeExportMenu();const script=buildFigmaScripterScript($('frame'),$('answer'),scenes[scene]);try{await navigator.clipboard.writeText(script);notify('Figma Scripter 脚本已复制。');}catch{const blob=new Blob([script],{type:'text/javascript;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`ai-answer-${scene}-figma-scripter.js`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notify('脚本已下载，可复制到 Figma Scripter 运行。');}}
...
function updateCompare(){const divider=$('compare-divider'),old=$('answer-legacy'),fresh=$('answer'),frame=$('frame'),style=getComputedStyle(frame),pt=parseFloat(style.paddingTop)||0,pr=parseFloat(style.paddingRight)||0,pb=parseFloat(style.paddingBottom)||0,pl=parseFloat(style.paddingLeft)||0;old.style.display=compare?'block':'none';old.style.inset=`${pt}px ${pr}px ${pb}px ${pl}px`;old.style.padding='0';if(!compare){frame.classList.remove('is-comparing');divider.style.left='50%';fresh.style.clipPath='';old.style.clipPath='';return;}frame.classList.add('is-comparing');dividerPositionFromContent();fresh.style.clipPath=`inset(0 0 0 ${divider.dataset.position||50}%)`;old.style.clipPath=`inset(0 ${100-(Number(divider.dataset.position||50))}% 0 0)`;divider.querySelector('span').textContent='旧 ｜ 新';}
function setComparePosition(x){const frame=$('frame'),r=frame.getBoundingClientRect(),pad=parseFloat(getComputedStyle(frame).paddingLeft)||0,contentWidth=Math.max(1,r.width-pad-(parseFloat(getComputedStyle(frame).paddingRight)||0)),p=Math.max(0,Math.min(100,((x-r.left-pad)/contentWidth)*100));$('compare-divider').dataset.position=p.toFixed(2);updateCompare();}
function dividerPositionFromContent(){const frame=$('frame'),divider=$('compare-divider'),r=frame.getBoundingClientRect(),pad=parseFloat(getComputedStyle(frame).paddingLeft)||0,contentWidth=Math.max(1,r.width-pad-(parseFloat(getComputedStyle(frame).paddingRight)||0)),p=Number(divider.dataset.position||50);divider.style.left=(pad+contentWidth*p/100)+'px';}
...
$('export-menu-button').addEventListener('click',event=>{event.stopPropagation();const menu=$('export-menu-list'),open=menu.hidden;menu.hidden=!open;$('export-menu-button').setAttribute('aria-expanded',String(open));});
$('export-menu-list').addEventListener('click',event=>event.stopPropagation());
$('export-jpg').addEventListener('click',()=>{closeExportMenu();exportPreviewJPG();});
$('export-figma').addEventListener('click',exportToFigma);
...
const previewHeading=document.querySelector('[data-view="preview"] > .panel-heading'),previewActions=previewHeading.querySelector('.preview-actions'),tabs=document.querySelector('.content-tabs');previewActions.classList.add('tab-preview-actions');tabs.append(previewActions);previewHeading.remove();
renderVersions();renderControls(false);renderStyle();renderContent();
document.querySelectorAll('.content-tab').forEach(tab=>tab.addEventListener('click',()=>{document.querySelectorAll('.content-tab').forEach(t=>t.classList.toggle('active',t===tab));document.querySelectorAll('.tab-view').forEach(v=>v.classList.toggle('active',v.dataset.view===tab.dataset.tab));previewActions.hidden=tab.dataset.tab!=='preview';}));
