...
function updateCompare(){const divider=$('compare-divider'),old=$('answer-legacy'),fresh=$('answer'),frame=$('frame');old.style.display=compare?'block':'none';old.style.padding=getComputedStyle(frame).padding;if(!compare){frame.classList.remove('is-comparing');divider.style.left='50%';fresh.style.clipPath='';old.style.clipPath='';return;}frame.classList.add('is-comparing');dividerPositionFromContent();fresh.style.clipPath=`inset(0 0 0 ${divider.dataset.position||50}%)`;old.style.clipPath=`inset(0 ${100-(Number(divider.dataset.position||50))}% 0 0)`;divider.querySelector('span').textContent='旧 ｜ 新';}
function setComparePosition(x){const frame=$('frame'),r=frame.getBoundingClientRect(),pad=parseFloat(getComputedStyle(frame).paddingLeft)||0,contentWidth=Math.max(1,r.width-pad-(parseFloat(getComputedStyle(frame).paddingRight)||0)),p=Math.max(5,Math.min(95,((x-r.left-pad)/contentWidth)*100));$('compare-divider').dataset.position=p.toFixed(2);dividerPositionFromContent();updateCompare();}
